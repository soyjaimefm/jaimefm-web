...
export default function blogPage() {
  const postsData = getAllFilesMetadata().filter((post) => post.published === true);
  return (
    <section className="">
            ...
            postsData.map(post => (
              <article key={post.slug} className="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex items-start justify-between flex-col w-full">
                ...
                <div className="p-5">
                  ...
                  <Link href={'/blog/' + post.slug} className="inline-flex items-center justify-center px-4 py-2 mr-3 text-base font-medium text-center text-gray-100 rounded-md bg-darkblue hover:opacity-70">
                    Leer más
                  </Link>
                </div>
              </article>
